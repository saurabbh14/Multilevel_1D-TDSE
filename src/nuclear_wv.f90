...
contains
    ...
    subroutine open_files(this)
        ...
        Nloop: do j = 1, Nstates ! varying the different adiabatic states
            ...
            open(newunit=this%vib_en_tk(j),file=filepath,status='unknown')
            write(filepath,'(a,a,i0,a)') adjustl(trim(output_data_dir)), &
                & "BO_Electronic-state-g", int(j-1), "_chi0-Evib.out"
            open(newunit=this%chi0_vib_en_tk(j),file=filepath,status='unknown')

            write(filepath,'(a,a,i0,a)') adjustl(trim(output_data_dir)), &
              & "BO_Electronic-state-g", int(J-1), "_vibstates.out"
            open(newunit=this%chi0_tk(j),file=filepath,status='unknown')
    ...
    subroutine open_not_converged_files(this, v, j)
        ...
        open(newunit=this%not_converged_tk,file=filepath,status='unknown')
